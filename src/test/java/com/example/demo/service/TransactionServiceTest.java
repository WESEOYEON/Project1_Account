...
class TransactionServiceTest {
    ...
    @Test
    @DisplayName("계좌 소유주가 다를 때 - 계좌 해지 실패")
    ...
    @Test
    @DisplayName("해지 계좌는 재해지 할 수 없다 - 계좌 해지 실패")
    ...
    void cancelTransaction_AccountNotFound() {
        ...
        //when
        AccountException exception = assertThrows(AccountException.class,
                () -> transactionService.cancelBalance("transaction", "1234567890", 1000L));
        //then
        assertEquals(ErrorCode.ACCOUNT_NOT_FOUNT, exception.getErrorCode());
    }
    ...
    void successQueryTransaction() {
        ...
        given(transactionRepository.findByTransactionId(anyString()))
                .willReturn(Optional.of(transaction));
        //when
        TransactionDto transactionDto = transactionService.queryTransactions("trxId");
        //then
        assertEquals(USE, transactionDto.getTransactionType());
        assertEquals(S, transactionDto.getTransactionResultType());
        assertEquals(1000L, transactionDto.getAmount());
